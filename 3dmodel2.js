...
camera.position.set(0.05, -0.1, 0.7
); // cerca del modelo
...
loader.load(
  'Kapsula.glb',  // archivo del modelo
  (gltf) => {
    model = gltf.scene;
...
const onWheel = (e) => {
  ...
  if (camera.position.z < 0.6) camera.position.z = 0.6; // muy cerca
  if (camera.position.z > 1.1) camera.position.z = 1.1;       // muy lejos
